...
function WalletIllustration() {
  ...
  return (
    <svg viewBox="0 0 200 140" className="w-full h-full" xmlns="http://www.w3.org/2000/svg">
      ...
      <g clipPath="url(#wc)">
        <path d={`M${pts.map(([x, y]) => `${x},${y}`).join(" L")} L${pts[pts.length-1][0]},123 L${pts[0][0]},123 Z`} fill="url(#wg1)" opacity="0.07" />
        <polyline points={pts.map(([x, y]) => `${x},${y}`).join(" ")} fill="none" stroke="url(#wg1)" strokeWidth="2" strokeLinecap="round" strokeLinejoin="round" opacity="0.85" />
        {pts.map(([x, y], i) => <circle key={i} cx={x} cy={y} r="2.5" fill="url(#wg1)" opacity="0.9" />)}
      </g>
...
const cases: Case[] = [
  {
    ...
    problem: "Mining users accumulated crypto but had no way to spend it in real life. This created tens of millions in monthly fiat outflow leaving the platform.",
    signal: "A 'coming soon' landing generated 4,000 signups from existing users and 1,700 new users — strong demand signal before writing a line of product code.",
    solution: "Full KYC flow. Rejected paid channels (CAC $110+). CRM-driven activation (21% rate) and referral program (LTV/CAC 2.8:1).",
    result: "10,000+ active users in 5 months, $MM+ in transaction volume, 60% payback period. Durable growth engine without paid budget.",
    lesson: "Outsourcing compliance means outsourcing control over your activation funnel. The partner's KYC flow directly determined our conversion rate.",
  ...
  },
  {
    ...
    problem: "Single revenue stream while all top competitors had fiat on/off-ramp. Without fiat access, we were losing users at the top of the funnel.",
    signal: "34% of users were already using external P2P services discovered from our platform — they wanted fiat and were finding it elsewhere.",
    solution: "Integrated Mercuryo as a white-label widget instead of building own infrastructure. No KYC, no licenses, launched in weeks.",
    result: "$4.2M+ monthly volume. ROI achieved in 3 months. Proved market fit before deciding whether to build in-house.",
    lesson: "Speed of validation beats control over UX at early stage. Own the experience later — first prove conversion.",
  ...
  },
  {
    ...
    problem: "Active base growing but users held crypto in wallet without converting to monetized products. Churn left without revenue conversion.",
    signal: "1,400 new active users/month via phone and email transfers — an organic acquisition channel we weren't leveraging.",
    solution: "CRM cross-activation into card, on/off-ramp, and Coinhold earn. Referral program with $ payout per activation. Expanded ramp with new coins and pairs.",
    result: "MAU 30K→42K (+40%), 400K total users, LTV +11%, 3-month retention +4%, 1,400 new active users/month.",
    lesson: "Retention without monetization is just cost. Cross-product CRM activation outperformed every paid channel tested.",
...
/* ─── Case Item (always expanded) ───────────────────────────── */

function CaseItem({ c, index }: { c: Case; index: number }) {
  ...
  return (
    ...
    >
      ...
      <div className="grid md:grid-cols-[1fr_220px] gap-10 items-start">
        ...
        {/* illustration */}
        <div className="hidden md:flex items-center justify-center rounded-2xl border border-white/5 bg-white/[0.02] p-5 aspect-square">
          {c.illustration}
        </div>
      </div>
    </div>
